...
import { useGetEvidences } from "./_hooks/use-get-list-evidence";

const EvidencePage: FC = (): ReactElement => {
  const [page, setPage] = useState(1);
  console.log('CEK DATA', setPage)
  const { data, isLoading } = useGetEvidences({ page, limit: 10 });
  const items = data?.data?.items || [];

  return (
    <Page>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Typography variant="h5" fontWeight="bold">Daftar Evidence</Typography>
